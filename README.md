...
flowchart TD
    ...
    Anemometer --> RS485
    Lux[Lux Meter] --> RS485
    Hum[Humidity Sensor] --> RS485
    GPS[GPS Module] --> MCU
    Soil[Soil Salinity Sensor] --> RS485
    RS485 --> MCU
    MCU -->|Web Socket| Dashborad
    MCU -->|MQTT| Controller_Sensor
    MCU -->|Flash| LocalStorage
    MCU --> internet[4G Module]
    MCU -->Opto_Controller_Sensor
    Solar_Cell --> Solar_Charger
    Solar_Charger --> Batterry
    Batterry --> MCU
    MCU --> test_Batterry
    test_Batterry --> Batterry
```

#### Phase 1: Data Sensing – การตรวจวัดข้อมูล

ระบบเริ่มต้นจากการตรวจวัดข้อมูลภาคสนามผ่านเซ็นเซอร์หลายประเภท ได้แก่

* เซ็นเซอร์วัดปริมาณน้ำฝน (Rain Sensor)

*  เซ็นเซอร์วัดความเร็วลม (Anemometer)

* เซ็นเซอร์วัดความเข้มแสง (Lux Meter)

* เซ็นเซอร์วัดความชื้นอากาศ (Humidity Sensor)

* เซ็นเซอร์วัดความเค็มของดิน (Soil Salinity Sensor)

* ข้อมูลจากเซ็นเซอร์เหล่านี้ถูกส่งผ่านบัสสื่อสาร RS485 ซึ่งเหมาะกับการใช้งานในระยะไกลและสภาพแวดล้อมภาคสนาม ลดสัญญาณรบกวน และรองรับอุปกรณ์หลายตัวในสายเดียว

#### Phase 2: Data Aggregation – การรวมและจัดการข้อมูล

ข้อมูลจาก RS485 และตำแหน่งพิกัดจาก GPS Module จะถูกส่งเข้าสู่ MCU (Microcontroller Unit) ซึ่งทำหน้าที่เป็นแกนกลางของระบบ MCU จะดำเนินการ:
...
* กำหนดรอบเวลาในการเก็บและส่งข้อมูล

ในกรณีที่การเชื่อมต่อเครือข่ายขัดข้อง ระบบสามารถบันทึกข้อมูลลง Flash / Local Storage เพื่อป้องกันข้อมูลสูญหาย

#### Phase 3: Communication – การส่งข้อมูลผ่านเครือข่าย 4G

เมื่อข้อมูลถูกประมวลผลแล้ว MCU จะส่งข้อมูลผ่าน 4G Module ไปยังระบบปลายทาง โดยรองรับการสื่อสารหลายรูปแบบ ได้แก่

* MQTT สำหรับการส่งข้อมูลเซ็นเซอร์แบบเบาและมีประสิทธิภาพ

* WebSocket สำหรับการอัปเดตข้อมูลแบบ real-time ไปยัง Dashboard

แนวทางนี้ช่วยให้ระบบสามารถรองรับทั้งการเก็บข้อมูลระยะยาวและการแสดงผลสดได้พร้อมกัน

#### Phase 4: Monitoring & Control – การแสดงผลและควบคุม

ข้อมูลที่ถูกส่งไปยังฝั่งเซิร์ฟเวอร์จะถูกแสดงผลผ่าน Dashboard เพื่อให้ผู้ใช้งานสามารถ:

* ติดตามค่าความเค็มของดินและสภาพอากาศ

* ตรวจสอบสถานะอุปกรณ์

* วิเคราะห์แนวโน้มข้อมูลตามเวลา

นอกจากนี้ ระบบยังสามารถส่งคำสั่งควบคุมกลับไปยังอุปกรณ์ผ่าน Controller Sensor / Opto Controller เพื่อรองรับการขยายไปสู่ระบบควบคุมอัตโนมัติในอนาคต

#### Phase 5: Power Management – การจัดการพลังงาน

ระบบใช้พลังงานจาก Solar Cell ร่วมกับ Solar Charger เพื่อชาร์จและจัดการพลังงานให้กับ Battery ภายในระบบ มีการตรวจสอบสถานะแบตเตอรี่ (test_Battery) เพื่อให้ระบบสามารถทำงานได้อย่างต่อเนื่องในพื้นที่ห่างไกล ลดการพึ่งพาแหล่งพลังงานภายนอก

#### สรุปของระบบ

ระบบ IoT นี้มีลำดับการทำงานตั้งแต่
การตรวจวัด → ประมวลผล → สื่อสาร → แสดงผล → จัดการพลังงาน
โดยออกแบบให้เหมาะกับการใช้งานจริงในภาคสนาม ใช้เครือข่าย 4G เพื่อความครอบคลุม และรองรับการขยายระบบในอนาคต เช่น การวิเคราะห์ข้อมูลหรือการควบคุมอัตโนมัติ
